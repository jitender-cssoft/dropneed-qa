...
Class order_model extends CI_Model {
    ...
    function get_gross_monthly_sales($year, $vendorId = 0) {
        ...
        $this->db->from('orders');
        //$this->db->join('customers', 'customers.');
    ...
    function get_orders($search = false, $sort_by = '', $sort_order = 'DESC', $limit = 0, $offset = 0, $customer_id = 0, $group = false) {
        ...
        $this->db->join('customers AS c', 'oi.venture_id = c.id');
    ...
    function save_order($data, $contents = false) {
        ...
        if ($contents) {
            ...
            foreach ($contents as $item) {
                ...
                $productDtl = $this->product_model->get_product($item['id']);
                
                $save['venture_id'] = $productDtl->added_by_cust;
    ...
    function get_venture_orders($arrVentures){
        $this->db->join('order_items', 'orders.id = order_items.order_id');
        $this->db->where_in('order_items.venture_id', $arrVentures);
